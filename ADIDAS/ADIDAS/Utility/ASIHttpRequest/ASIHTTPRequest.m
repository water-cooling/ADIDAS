...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setShouldAttemptPersistentConnection:NO];//把ASIHttpRequest的连接重用关闭 jerry.lu 2012.07.19
    ...
    @try {
        ...
        NSData *PKCS12Data = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:kCertName ofType:@"pfx"]];//证书文件名和文件类型
        [self extractIdentity:&identity andTrust:&trust fromPKCS12Data:PKCS12Data];
        [self setClientCertificateIdentity:identity];//设定访问路径
        [self setValidatesSecureCertificate:NO];//是否验证服务器端证书，如果此项为yes那么服务器端证书必须为合法的证书机构颁发的，而不能是自己用openssl 或java生成的证书
...
- (BOOL)extractIdentity:(SecIdentityRef *)outIdentity andTrust:(SecTrustRef*)outTrust fromPKCS12Data:(NSData *)inPKCS12Data {
    ...
    @try {
        ...
        CFStringRef password = kCertPass; //证书密码
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if (delegate&& [[self delegate] respondsToSelector:[self didReceiveDataSelector]]) {

...
-(void)cancelDownload:(NSString *)resourceUrl
{
    ...
    if (downItem == nil)
    {
        ...
        NSArray* filearr = [resourceUrl componentsSeparatedByString:@"/"];
        downItem=[[DownloadItem alloc]init];
        downItem.url=[NSURL URLWithString:resourceUrl];
        NSString *desPath=[[Utilities CachePath] stringByAppendingPathComponent:[filearr lastObject]];
        downItem.temporaryFileDownloadPath = desPath;
        [downItem cancelDownloadTask];
        [[NSNotificationCenter defaultCenter]postNotificationName:@"DownloadManagerNotification" object:downItem userInfo:nil];
    }

...
@interface ComponentViewController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate,DeleteSelectImageDelegate>
...
- (void)creatNormalNoStickerShoesSZData{
    ...
    model4.cellType = CreateTableTFDownCell;
    model4.title = @"渠道：";
    ...
    model12.cellType = CreateTablePicCell;
    model12.title = @"";
    model12.key = @"";
    [self.dataArray addObject:model12];
    
    CreatTableModel *model13 = [CreatTableModel new];
    model13.cellType = CreateTableReMarkCell;
    model13.title = @"";
    model13.placeholder = @"请输入申请原因";
    model13.key = @"CaseReason";
...
-(void)sheetAler:(NSString *)title indexPath:(NSIndexPath *)indexPath{
        ...
        }else if ([title isEqualToString:@"SecurityCategory"]){
            [self getSecurityCategory:indexPath];
        }else if ([title isEqualToString:@"ArticleGrade"]){
            [self GetArticleGrade:indexPath];
        }
...
-(void)showPoint:(CreatTableModel *)model{
    ...
    if ([model.key isEqualToString:@"MaterialComposition"]) {
                for (NSDictionary * dic in self.sampleArr) {
                    if ([dic[@"SampleType"] isEqualToString:@"MaterialComposition"]) {
                        [self.pointView configTitle:model.pointTitle destitlte:model.pointDesTitle pic:dic[@"PictureUrl"]];
                    return;
                }
            }
        }
    if ([model.key isEqualToString:@"Attention"]) {
                for (NSDictionary * dic in self.sampleArr) {
                    if ([dic[@"SampleType"] isEqualToString:@"Attention"]) {
                        [self.pointView configTitle:model.pointTitle destitlte:model.pointDesTitle pic:dic[@"PictureUrl"]];
                    return;
                }
            }
